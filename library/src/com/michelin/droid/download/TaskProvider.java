...
public class TaskProvider
{
  ...
  static void deleteTask(Context paramContext, DownloadTask paramDownloadTask)
  {
    ...
    excuteSQL(paramContext, "delete from download_task where resourceId = ? and name = ?", arrayOfObject);
  }

  private static void excuteSQL(Context paramContext, String paramString, Object[] paramArrayOfObject)
  {
    SQLiteDatabase localSQLiteDatabase = null;
    try
    {
      localSQLiteDatabase = new PandaSpaceDatabaseHelper(paramContext, "pandaspaceDB", null, 6).getWritableDatabase();
  ...
  static void insertTask(Context paramContext, DownloadTask paramDownloadTask)
  {
    ...
    arrayOfObject[0] = paramDownloadTask.resourceId;
    arrayOfObject[1] = paramDownloadTask.name;
    arrayOfObject[2] = paramDownloadTask.size;
    ...
    excuteSQL(paramContext, "insert into download_task (resourceId,name,size,path,url,logo_url,pkg_name,version_name,start_time,res_type) values(?,?,?,?,?,?,?,?,?,?)", arrayOfObject);
  }

  static List<DownloadTask> loadRunningTasks(Context paramContext)
  {
    return loadTask(paramContext, "select resourceId,name,size,path,url,logo_url,pkg_name,version_name,res_type from download_task where end_time is null order by start_time desc", false);
  }

  private static List<DownloadTask> loadTask(Context paramContext, String paramString, boolean paramBoolean)
  {
    ...
    try
    {
      ...
      if (localCursor.moveToFirst())
        do
        {
          ...
          localDownloadTask.size = Long.parseLong(localCursor.getString(2));
          ...
          localDownloadTask.loadSize = localDownloadTask.getCurrentSize();
          localDownloadTask.percent = localDownloadTask.caculatePercent();
          if (paramBoolean)
          {
  ...
  static void updateTaskSizeAndPath(Context paramContext, DownloadTask paramDownloadTask)
  {
    Object[] arrayOfObject = new Object[4];
    arrayOfObject[0] = paramDownloadTask.size;
    arrayOfObject[1] = paramDownloadTask.path;
    arrayOfObject[2] = paramDownloadTask.resourceId;
    arrayOfObject[3] = paramDownloadTask.name;
    excuteSQL(paramContext, "update download_task set size=?, path =?  where resourceId = ? and name = ?", arrayOfObject);
  }
}