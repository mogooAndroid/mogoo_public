...
public class ResourceUtility {
	...
	public static void changeDirectoryPrivilege(String path) {
		try {
			if (!isPrivateStoreDir(path))
				return;
			Object localObject = path;
			String str1 = new File(SystemVal.private_file_dir).getParent();
			if (((String) localObject).equals(str1))
				return;
			changeFilePrivilege((String) localObject);
			String str2 = new File((String) localObject).getParent();
			localObject = str2;
	...
	private static boolean isPrivateStoreDir(String s) {
		if (s != null && s.startsWith(SystemVal.private_file_dir))
			return true;
		else
			return false;
	}
	
	public static void changeFilePrivilege(String paramString) {
		try {
			if (isPrivateStoreDir(paramString)) {
				String[] arrayOfString = new String[3];
				arrayOfString[0] = "chmod";
				arrayOfString[1] = "777";
